...
class GPT
  ...
  def initialize(slack_events_api_handler:, response_channel:)
    ...
    $logger.debug "OpenAI temperature: #{temperature}"
  end
  ...
  def build_chat_messages_list(conversation_history)
    ...
    token_limit = (model_max_tokens / 2) # Half the max tokens
    ...
    until (
      (current_token_estimate = conversation_history.sum{|message|
        message['estimatedOpenAiTokenCount'].to_i }) <
          token_limit
    ) do
      # If the conversation history is too long, remove the oldest message
      # and try again.
      $logger.warn "Trimming conversation history because the current token estimate of #{current_token_estimate} is greater than the token limit of #{token_limit}."
      conversation_history =
    ...
    def api_response(conversation_history:,
      ...
      @open_ai_client.chat(
        parameters: {
          ...
          @cloudwatch_metrics.send_metric_reading(
            metric_name: "Open AI Temperature",
            value: temperature * 1,
            unit: 'Count'
          )
    ...
    response = 
      ...
      if response_message['function_call'].present?
        ...
        response_message['content'].tap do |response|
          $logger.debug "OpenAI response message content: #{response}"
      end
    end
  end
